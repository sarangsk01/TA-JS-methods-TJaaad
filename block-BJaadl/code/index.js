...
function checkVowel(words){
  return words.toLowerCase().includes('a') || words.toLowerCase().includes('e') || words.toLowerCase().includes('i') || words.toLowerCase().includes('o') || words.toLowerCase().includes('u')
}
...
function averageWordLength(words){
  let wordsLength = words.map((w) => w.length).reduce((acc, cv) => {
    return acc + cv;
  },0) / words.length
